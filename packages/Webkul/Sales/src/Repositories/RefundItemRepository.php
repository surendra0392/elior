        ...
        if (
            ...
            if (($totalShippedQtyToRefund = $quantity - $nonShippedQty) > 0) {
                ...
                foreach ($shipmentItems as $shipmentItem) {
                    ...
                    if ($inventory && $shippedQtyToRefund > 0) {
                        \Webkul\Inventory\Models\InventoryMovementProxy::modelClass()::create([
                            'product_id' => $product->id,
                            'inventory_source_id' => $shipmentItem->shipment->inventory_source_id,
                            'quantity' => $shippedQtyToRefund,
            ...
            if ($inventory && $quantity > 0) {
                \Webkul\Inventory\Models\InventoryMovementProxy::modelClass()::create([
                    ...
                    'reference_id' => $orderItem->order_id,
                    'user_id' => auth()->guard('admin')->check() ? auth()->guard('admin')->user()->id : null,
                    'notes' => 'Refund created (virtual/bundled return)',
                ]);
