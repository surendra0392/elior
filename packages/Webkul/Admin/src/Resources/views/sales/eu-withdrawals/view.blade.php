@php
    ...
    $statusBadge = [
        WithdrawalStatus::RECEIVED => 'bg-amber-100 text-amber-800',
        WithdrawalStatus::REFUNDED => 'bg-emerald-100 text-emerald-800',
        WithdrawalStatus::DECLINED => 'bg-red-100 text-red-800',
    ][$withdrawal->status] ?? 'bg-gray-100 text-gray-800';

    $timelineDot = [
        'done'    => '<span class="mt-0.5 grid h-7 w-7 flex-shrink-0 place-items-center rounded-full bg-emerald-500 text-white shadow-sm ring-4 ring-emerald-100"><svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M16.704 5.292a1 1 0 010 1.414l-7.999 8a1 1 0 01-1.413 0l-4-3.999a1 1 0 011.413-1.415L8 12.585l7.295-7.293a1 1 0 011.41 0z" clip-rule="evenodd"/></svg></span>',

        'declined' => '<span class="mt-0.5 grid h-7 w-7 flex-shrink-0 place-items-center rounded-full bg-red-500 text-white shadow-sm ring-4 ring-red-100"><svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></span>',

        'warning' => '<span class="mt-0.5 grid h-7 w-7 flex-shrink-0 place-items-center rounded-full bg-amber-500 text-white shadow-sm ring-4 ring-amber-100"><svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg></span>',

        'pending' => '<span class="mt-0.5 grid h-7 w-7 flex-shrink-0 place-items-center rounded-full border-2 border-dashed border-gray-300 bg-white ring-4 ring-gray-100"></span>',
    ];
@endphp
...
<x-admin::layouts>
    ...
    <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
        <div class="flex items-center gap-2.5">
            <p class="py-3 text-xl font-bold leading-6 text-gray-800">
                @lang('admin::app.eu_withdrawal.view.heading')

                <span class="ml-1 font-mono text-base text-gray-500">
                    #{{ $withdrawal->id }}
                </span>
            </p>
    ...
    <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
        ...
        <div class="flex flex-1 flex-col gap-2.5 max-xl:flex-auto">
            ...
            <div class="box-shadow rounded bg-white">
                ...
                <div class="grid grid-cols-1 divide-y divide-slate-200 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                    ...
                    <div class="px-4 py-4">
                        ...
                        <div class="mt-1 flex items-center gap-2 rounded-lg border border-slate-200 bg-gray-50 px-3 py-2">
                            ...
                            <button
                                ...
                                class="grid h-7 w-7 flex-shrink-0 place-items-center rounded-md text-gray-500 transition-all hover:bg-gray-200 hover:text-gray-800"
                            >
                ...
                @if ($withdrawal->reason_text)
                    <div class="border-t border-slate-200 px-4 py-4">
                        ...
                        <p class="mt-1 whitespace-pre-wrap text-sm text-gray-800">{{ $withdrawal->reason_text }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="box-shadow rounded bg-white">
                ...
                <ol class="grid gap-0">
                    <li class="flex gap-3 border-t border-slate-200 px-4 py-3">
                        {!! $timelineDot['done'] !!}
                        ...
                        @if ($withdrawal->declined_at)
                            ...
                            <div class="flex-1">
                                ...
                                @if ($withdrawal->declined_reason)
                                    <div class="mt-2 rounded-md border border-red-100 bg-red-50 p-2.5">
                                        ...
                                        <p class="mt-1 text-sm text-red-900">{{ $withdrawal->declined_reason }}</p>
                                    </div>
                                @endif
                            </div>
                        @elseif ($withdrawal->refunded_at)
                            ...
                            <div class="flex-1">
                                ...
                                @if ($withdrawal->refund_note)
                                    <div class="mt-2 rounded-md border border-emerald-100 bg-emerald-50 p-2.5">
                                        ...
                                        <p class="mt-1 text-sm text-emerald-900">{{ $withdrawal->refund_note }}</p>
                                    </div>
                                @endif
                            </div>
                        @else
            ...
            <div class="box-shadow rounded bg-white">
                ...
                @if (bouncer()->hasPermission('sales.eu_withdrawals.mark_refunded'))
                    <form
                        ...
                        class="grid gap-2 border-t border-slate-200 p-4"
                    >
                        @csrf
                        ...
                        <input
                            ...
                            class="w-full rounded-lg border bg-white px-3 py-2 text-sm text-gray-700 transition-all hover:border-gray-400 focus:border-gray-400"
                        >
                ...
                @if (bouncer()->hasPermission('sales.eu_withdrawals.decline'))
                    ...
                    >
                        ...
                        <button
                            type="button"
                            class="secondary-button w-full !border-red-200 !text-red-700 hover:!bg-red-50!bg-red-900/30"
                            @click="
