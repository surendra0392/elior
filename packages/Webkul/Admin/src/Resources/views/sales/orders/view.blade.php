    ...
    <div class="grid">
        <div class="flex items-center justify-between gap-4 max-sm:flex-wrap">
            ...
            <div class="flex items-center gap-2.5">
                <p class="text-xl font-bold leading-6 text-gray-800">
                    @lang('admin::app.sales.orders.view.title', ['order_id' => $order->increment_id])
                </p>
                ...
                <div class="flex items-center ml-2 border-l pl-4">
                    <form action="{{ route('admin.sales.orders.update_operational_status', $order->id) }}" method="POST" class="flex items-center gap-2">
                        @csrf
                        <select name="operational_status" class="custom-select rounded-lg border-gray-300 px-3 py-1 text-sm shadow-sm transition-all focus:border-blue-500 focus:ring-blue-500">
                            <option value="PAYMENT PENDING" {{ $order->operational_status == 'PAYMENT PENDING' ? 'selected' : '' }}>Payment Pending</option>
                            <option value="PAYMENT CONFIRMED" {{ $order->operational_status == 'PAYMENT CONFIRMED' ? 'selected' : '' }}>Payment Confirmed</option>
                            <option value="PACKING" {{ $order->operational_status == 'PACKING' ? 'selected' : '' }}>Packing</option>
                            <option value="READY TO SHIP" {{ $order->operational_status == 'READY TO SHIP' ? 'selected' : '' }}>Ready to Ship</option>
                            <option value="SHIPPED" {{ $order->operational_status == 'SHIPPED' ? 'selected' : '' }}>Shipped</option>
                            <option value="DELIVERED" {{ $order->operational_status == 'DELIVERED' ? 'selected' : '' }}>Delivered</option>
                        </select>
                        <button type="submit" class="primary-button text-sm px-3 py-1">Update</button>
                    </form>
                </div>
            </div>
    ...
    <div class="mt-5 flex-wrap items-center justify-between gap-x-1 gap-y-2">
        <div class="flex gap-1.5">
            ...
            @if (
                ...
                <a
                    ...
                    class="transparent-button px-1 py-1.5 hover:bg-gray-200"
                >
            ...
            @if ($order->invoices->count())
                <a
                    href="{{ route('admin.sales.invoices.print', $order->invoices->first()->id) }}"
                    class="transparent-button px-1 py-1.5 hover:bg-gray-200"
                    target="_blank"
                >
            ...
            <a
                href="{{ route('admin.sales.orders.print', $order->id) }}"
                class="transparent-button px-1 py-1.5 hover:bg-gray-200"
                target="_blank"
            >
            ...
            <a
                href="{{ route('admin.sales.orders.delivery_slip.print', $order->id) }}"
                class="transparent-button px-1 py-1.5 hover:bg-gray-200"
                target="_blank"
            >
        ...
        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            ...
            <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
                ...
                <div class="box-shadow rounded bg-white">
                    ...
                    <div class="grid">
                        ...
                        @foreach ($order->items as $item)
                            ...
                            <div class="flex justify-between gap-2.5 border-b border-slate-300 px-4 py-6">
                                <div class="flex gap-2.5">
                                    @if($item?->product?->base_image_url)
                                        ...
                                        >
                                    @else
                                        <div class="relative h-[60px] max-h-[60px] w-full max-w-[60px] rounded border border-dashed border-gray-300">
                                    ...
                                    <div class="grid place-content-start gap-1.5">
                                        <p
                                            class="break-all text-base font-semibold text-gray-800"
                                            v-pre
                                        >
                                            {{ $item->name }}
                                        </p>
                                                    ...
                                                    >
                                                        ...
                                                        @else
                                                            ...
                                                            <a
                                                                ...
                                                                class="text-[#205132] hover:underline"
                                ...
                                <div class="grid place-content-start gap-1">
                                    <div class="">
                                        <p class="flex items-center justify-end gap-x-1 text-base font-semibold text-gray-800">
                        ...
                        <div class="grid max-w-max gap-2 text-sm">
                            ...
                            @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                <div class="flex w-full justify-between gap-x-5">
                                    <p class="font-semibold !leading-5 text-gray-600">
                            ...
                            @php
                                $storeState = core()->getConfigData('sales.shipping.origin.state') ?? 'MH'; // Defaulting to MH if unset
                                $customerState = $order->shipping_address ? $order->shipping_address->state : ($order->billing_address ? $order->billing_address->state : '');
                                
                                $isIgst = ($storeState !== $customerState);
                                $taxAmount = (float) $order->base_tax_amount;
                            @endphp

                            @if($taxAmount > 0)
                                @if($isIgst)
                                    <div class="flex w-full justify-between gap-x-5">
                                        <p class="!leading-5 text-gray-600">IGST</p>
                                        <p class="!leading-5 text-gray-600">{{ core()->formatBasePrice($taxAmount) }}</p>
                                    </div>
                                @else
                                    <div class="flex w-full justify-between gap-x-5">
                                        <p class="!leading-5 text-gray-600">CGST (50%)</p>
                                        <p class="!leading-5 text-gray-600">{{ core()->formatBasePrice($taxAmount / 2) }}</p>
                                    </div>
                                    <div class="flex w-full justify-between gap-x-5">
                                        <p class="!leading-5 text-gray-600">SGST (50%)</p>
                                        <p class="!leading-5 text-gray-600">{{ core()->formatBasePrice($taxAmount / 2) }}</p>
                                    </div>
                                @endif
                            @else
                                <div class="flex w-full justify-between gap-x-5">
                                    <p class="!leading-5 text-gray-600">@lang('admin::app.sales.orders.view.summary-tax')</p>
                                    <p class="!leading-5 text-gray-600">{{ core()->formatBasePrice($taxAmount) }}</p>
                                </div>
                            @endif
                            ...
                            <!-- Grand Total -->
                            <div class="flex w-full justify-between gap-x-5">
                                <p class="text-base font-semibold !leading-5 text-gray-800">
                ...
                <div class="box-shadow rounded bg-white">
                    <p class="p-4 pb-0 text-base font-semibold text-gray-800">
                        ...
                        <div class="p-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    ...
                                    <span
                                        class="icon-uncheckbox peer-checked:icon-checked cursor-pointer rounded-md text-2xl peer-checked:text-[#205132]"
                                        role="button"
                                        tabindex="0"
                                    >
                                    </span>

                                    <p class="flex cursor-pointer items-center gap-x-1 font-semibold text-gray-600 hover:text-gray-800">
                    ...
                    @foreach ($order->comments()->orderBy('id', 'desc')->get() as $comment)
                        <div class="grid gap-1.5 p-4">
                            <p 
                                class="break-all text-base leading-6 text-gray-800"
                                v-pre
                            >
                                {{ $comment->comment }}
                            </p>
                            ...
                            <p class="flex items-center gap-2 text-gray-600">
                                @if ($comment->customer_notified)
                                    <span class="icon-done h-fit rounded-full bg-emerald-100 text-2xl text-[#205132]"></span>
            ...
            <div class="flex w-[360px] max-w-full flex-col gap-2 max-sm:w-full">
                ...
                <x-admin::accordion>
                    <x-slot:header>
                        <p class="p-2.5 text-base font-semibold text-gray-600">
                ...
                <x-admin::accordion>
                    ...
                    <x-slot:content>
                        <div>
                            ...
                            <!-- Currency -->
                            <p 
                                class="pt-4 font-semibold text-gray-800"
                                v-pre
                            >
                        ...
                        @if ($order->shipping_address)
                            <span class="mt-4 block w-full border-b"></span>
                    ...
                    <x-slot:content>
                        @forelse ($order->invoices as $index => $invoice)
                            <div class="grid gap-y-2.5">
                                ...
                                <div class="flex gap-2.5">
                                    <a
                                        ...
                                        class="text-sm text-[#205132] transition-all hover:underline flex items-center gap-1"
                                    >
                            ...
                            @if ($index < count($order->invoices) - 1)
                                <span class="mb-4 mt-4 block w-full border-b"></span>
                            @endif
                        @empty
                            <p class="text-gray-600">
                        ...
                        @forelse ($order->shipments as $shipment)
                            <div class="grid gap-y-2.5">
                                ...
                                <div class="flex flex-wrap gap-2.5">
                                    ...
                                    <a
                                        href="{{ route('admin.sales.shipments.print', $shipment->id) }}"
                                        class="text-sm text-[#205132] transition-all hover:underline flex items-center gap-1"
                                        target="_blank"
                                    >
                                        <span class="icon-product text-lg"></span>
                                        Print Packing Slip
                                    </a>

                                    <a
                                        href="{{ route('admin.sales.shipments.delivery_slip.print', $shipment->id) }}"
                                        class="text-sm text-[#205132] transition-all hover:underline flex items-center gap-1"
                                        target="_blank"
                                    >
                                        <span class="icon-ship text-lg"></span>
                                        Print Delivery Slip
                                    </a>
                                </div>
                            </div>
                            ...
                            <div class="grid gap-y-2.5">
                                ...
                                <div class="flex gap-2.5">
                                    <a
                                        ...
                                        class="text-sm text-[#205132] transition-all hover:underline"
                                    >
                                        @lang('admin::app.sales.orders.view.view')
                                    </a>
                                </div>
