    ...
    public function store(TaxRateRequest $request)
    {
        ...
        if (is_array($states)) {
            foreach ($states as $state) {
                ...
                $taxRateData['state'] = $state;
                $taxRateData['identifier'] = $request->input('identifier') . '_' . $state;

                $taxRate = $this->taxRateRepository->create($taxRateData);
