...
class ProductController extends Controller
{
    ...
    /**
     * Toggle a specific boolean attribute for a product.
     *
     * @return JsonResponse
     */
    public function toggleAttribute(): JsonResponse
    {
        $productId = request()->input('product_id');
        $attribute = request()->input('attribute');
        $value = request()->input('value');

        if (! in_array($attribute, ['featured', 'best_sellers'])) {
            return new JsonResponse([
                'message' => trans('admin::app.catalog.products.invalid-attribute') ?? 'Invalid attribute',
            ], 400);
        }
