...
class CartResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        foreach ($this->items as $item) {
            $productPrice = (float) ($item->child?->product?->price ?? $item->product?->price ?? $item->price);
            $regularSubTotal += ($productPrice * $item->quantity);
        }

        $productDiscount = max(0, $regularSubTotal - (float) $this->sub_total);
        $totalSavings = $productDiscount + (float) $this->discount_amount;
        ...
        return [
            ...
            'regular_sub_total' => $regularSubTotal,
            'formatted_regular_sub_total' => core()->formatPrice($regularSubTotal),
            'product_discount' => $productDiscount,
            'formatted_product_discount' => core()->formatPrice($productDiscount),
            'total_discount' => $totalSavings,
            'formatted_total_discount' => core()->formatPrice($totalSavings),
            'has_discount' => ($totalSavings > 0.01),
            ...
            'billing_address' => $this->billing_address ? new AddressResource($this->billing_address) : null,
            'shipping_address' => $this->shipping_address ? new AddressResource($this->shipping_address) : null,
