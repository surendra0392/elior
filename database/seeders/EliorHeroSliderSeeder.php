...
class EliorHeroSliderSeeder extends Seeder
{
    ...
    public function run()
    {
        // 1. Validate Media Assets Existence
        $requiredAssets = [
            'theme/hero/elior/elior-hero-botanical-desktop.webp',
            'theme/hero/elior/elior-hero-botanical-mobile.webp',
            'theme/hero/elior/elior-hero-quality-desktop.webp',
            'theme/hero/elior/elior-hero-quality-mobile.webp',
            'theme/hero/elior/elior-hero-discovery-desktop.webp',
            'theme/hero/elior/elior-hero-discovery-mobile.webp',
        ];

        foreach ($requiredAssets as $assetRelPath) {
            $storagePath = storage_path('app/public/' . $assetRelPath);
            $publicPath = public_path('storage/' . $assetRelPath);

            if (! File::exists($storagePath) && ! File::exists($publicPath)) {
                throw new \RuntimeException(
                    "ERROR:\nELIOR hero asset missing:\n{$storagePath}\nPlease generate or copy media before seeding."
                );
            }
        }
